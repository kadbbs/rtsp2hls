...
int main(int argc, char* argv[]) {
  ...
  // 创建 HTTP 服务器
  std::unique_ptr<rtsp_rtmp2webrtc_hls::HTTPServer> http_server;
  if (enable_http) {
    rtsp_rtmp2webrtc_hls::HTTPServerConfig http_config;
    http_config.port = http_port;
    http_config.root_dir = config.hls_output_dir;
    http_server = std::make_unique<rtsp_rtmp2webrtc_hls::HTTPServer>(http_config);
    http_server->set_log_callback(log_cb);
  }
  ...
  std::cout << "  HTTP 端口：" << (enable_http ? std::to_string(http_port) : "禁用") << "\n";
  std::cout << "\n推流地址示例:\n";
  ...
  std::cout << "\n播放地址:\n";
  if (enable_http) {
    std::cout << "  HTTP 播放：http://localhost:" << http_port << "/\n";
  }
  std::cout << "\n按 Ctrl+C 停止\n" << std::endl;
